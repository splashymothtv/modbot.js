  ...
  async execute(interaction, client) {
    ...
    if (
      !interaction.member.permissions.has(
        PermissionsBitField.Flags.UnbanMembers
      )
    )
      return await interaction.reply({
        content: `You do not have the permission required to use this command!`,
        ephemeral: true,
      });
    ...
    await interaction.guild.bans.fetch().then(async (bans) => {
      if (bans.size === 0)
        return await interaction.reply({
          content: "No one has been banned in this server!",
          ephemeral: true,
        });
      let bannedID = bans.find((ban) => ban.user.id == userID);
      if (!bannedID)
        return await interaction.reply({
          content: `${userID} has not been banned in this server!`,
          ephemeral: true,
        });

      await interaction.guild.bans.remove(userID, reason).catch((err) => {
        return interaction.reply({
          content: `I am having trouble unbanning ${userID}`,
          ephemeral: true,
        });
      });
    });
