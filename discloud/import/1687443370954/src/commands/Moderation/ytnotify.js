...
module.exports = {
    data: new SlashCommandBuilder()
        ...
        .addSubcommand(command => command.setName('add').setDescription('Set up the yt notif system').addStringOption(option => option.setName('channel-id').setDescription('The ID of your YouTube Channel').setRequired(true)).addChannelOption(option => option.setName('channel').setDescription('The channel to send the notif in').setRequired(true).addChannelTypes(ChannelType.GuildAnnouncement)).addStringOption(option => option.setName('message').setDescription('A custom message for your notification. Use {author}, {link}, and {title}').setRequired(false)).addRoleOption(option => option.setName('role').setDescription('The role you want to be pinged').setRequired(false)))
        .addSubcommand(command => command.setName('remove').setDescription('Remove a channel from the YT Notif System').addStringOption(option => option.setName('channel-id').setDescription('The ID of the youtube channel you want to remove').setRequired(true))),
        ...
        switch (sub) {
            case 'add':
                ...
                try {
                    ...
                    if (message) {
                        embed.addFields(
                            { name: 'Message', value: `${message.replace('{author}', author).replace('{link}', 'youtube.com').replace('{title}', 'title')}` });
                    }
